var fs = require('fs')
	,reminders = require('../database/reminders.json')
	,updatedR = false;
...
function updateRemindDB() {
	fs.writeFile(__dirname + '/../database/reminders-temp.json', JSON.stringify(reminders), error=>{
		if (error) console.log(error)
		else {
			fs.stat(__dirname + '/../database/reminders-temp.json', (err, stats)=>{
				if (err) console.log(err)
				else if (stats["size"] < 2) console.log('Prevented reminders database from being overwritten');
				else {
					fs.rename(__dirname + '/../database/reminders-temp.json', __dirname + '/../database/reminders.json', e=>{if(e)console.log(e)});
