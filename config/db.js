...
function updateServers() {
	fs.writeFile(__dirname + '/../database/servers-temp.json', JSON.stringify(ServerSettings), error=>{
		if (error) console.log(error)
		else {
			fs.stat(__dirname + '/../database/servers-temp.json', (err, stats)=>{
				if (err) console.log(err)
				else if (stats["size"] < 5) console.log('Prevented server settings database from being overwritten');
				else {
					fs.rename(__dirname + '/../database/servers-temp.json', __dirname + '/../database/servers.json', e=>{if(e)console.log(e)});
...
function updateTimes() {
	fs.writeFile(__dirname + '/../database/times-temp.json', JSON.stringify(Times), error=>{
		if (error) console.log(error)
		else {
			fs.stat(__dirname + '/../database/times-temp.json', (err, stats)=>{
				if (err) console.log(err)
				else if (stats["size"] < 5) console.log('Prevented times database from being overwritten');
				else {
					fs.rename(__dirname + '/../database/times-temp.json', __dirname + '/../database/times.json', e=>{if(e)console.log(e)});
